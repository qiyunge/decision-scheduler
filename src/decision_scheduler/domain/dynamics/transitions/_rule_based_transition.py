...
from decision_scheduler.domain.decisions.action import Action, WaitAction, DispatchAction
from decision_scheduler.domain.semantics.events import DomainEvent, TimeAdvancedEvent
...
class AdvanceRule(Protocol):
    ...
    def apply(self, state: SchedulingState) -> list[DomainEvent]: ...
...
class IdleJumpToNextReleaseRule:
    ...
    def apply(self, state: SchedulingState) -> list[DomainEvent]:
        ...
        if rec is None:
            return []
        return [TimeAdvancedEvent(from_time=rec.from_time, to_time=rec.to_time)]
